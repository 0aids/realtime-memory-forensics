...
int main()
{
    ...
    auto filteredMaps = breakIntoRegionChunks(maps.filterRegionsByHasPerms("rwp"), 0);
    MTBackend backend(5);
    {
        auto snaps1 = backend.makeSnapshots(asSpan(filteredMaps));
        rmf_assert(snaps1.size() > 0, "There should be snapshots taken");
        this_thread::sleep_for(50ms);
        auto snaps2 = backend.makeSnapshots(filteredMaps);
        rmf_assert(snaps2.size() > 0, "There should be snapshots taken");
        auto spans1 = mt::makeSnapshotSpans(snaps1);
        auto spans2 = mt::makeSnapshotSpans(snaps2);
        auto changedRegionsResults = backend.findChangingRegions(asSpan(spans1), asSpan(spans2), 8);
        rmf_assert(changedRegionsResults.size() > 0, "There is a changing region in there");
        auto strResults= backend.findString(asSpan(spans1), asSpan(spans2), "Lorem ipsum");
        rmf_assert(strResults.size() > 0, "There is a Lorem ipsum in there");
    }
}
