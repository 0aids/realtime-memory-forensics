...
int main()
{
    ...
    auto writeRegions =
        map.m_mapSnapshots_l.back().getRegionsWithPermissions("rwp");
    Log(Message,
    ...
    for (const auto& region : writeRegions)
    {
        ...
        MemoryRegion mregion(region, pid);
        mregion.snapshot();
        auto        s        = mregion.getLastSnapshot();
        std::string sequence = "Lorem ipsum";
        auto it = std::search(s->begin(), s->end(), sequence.begin(),
                              sequence.end());
        if (it != s->end())
        {
            Log(Message,
                "Found lorem ipsum at: \n"
                    << region.toConstStr());
            total++;
        }
    }
    ...
    assert(total > 0,
           "There should be a lorem ipsum in their somewhere.");
    ...
    std::string sequence = "Lorem ipsum";
    auto        it       = std::search(snap->begin(), snap->end(),
