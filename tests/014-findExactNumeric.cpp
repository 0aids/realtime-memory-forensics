...
int main() {
    ...
    size_t diffCount = 0;
    rmf_Log(rmf_Info, "Testing exact numeric search.");
    for (auto &map: maps) {
        auto snap1 = MemorySnapshot::Make(map);
        auto results = findNumeralExact<double>(snap1, 150);
        if (results.size() > 0) {
            rmf_Log(rmf_Info, "Found double!");
            diffCount += results.size();
        }
    }
    rmf_Log(rmf_Info, "Number of exact: " << diffCount);
    rmf_Assert(diffCount > 0, "There is defnitel a double =150");

    diffCount = 0;
    rmf_Log(rmf_Info, "Testing exact numeric search (intended failure).");
    for (auto &map: maps) {
        auto snap1 = MemorySnapshot::Make(map);
        auto results = findNumeralExact<double>(snap1, -69);
        if (results.size() > 0) {
    ...
    rmf_Log(rmf_Info, "Number of exact: " << diffCount);
    rmf_Assert(diffCount == 0, "There should not be any -69 doubles...");
}
