...
TEST(findChanged, Empty)
{
    auto [m1, s1] = makeMRV({});
    auto [m2, s2] = makeMRV({});
    auto r        = rmf::findChanged(m1, s1, m2, s2, 1);
    EXPECT_TRUE(r.empty());
}

TEST(findChanged, Identical)
{
    auto [m1, s1] = makeMRV({1, 2, 3, 4});
    auto [m2, s2] = makeMRV({1, 2, 3, 4});
    auto r        = rmf::findChanged(m1, s1, m2, s2, 1);
    EXPECT_TRUE(r.empty());
}
...
{
    auto [m1, s1] = makeMRV({0, 0, 0, 0, 0, 0, 0, 0});
    auto [m2, s2] = makeMRV({1, 1, 1, 1, 1, 1, 1, 1});
    auto r        = rmf::findChanged(m1, s1, m2, s2, 1);
    ASSERT_EQ(r.size(), 1);
...
TEST(findChanged, CompareSizeMatches)
{
    auto [m1, s1] = makeMRV({0, 0, 0, 0, 0, 0, 0, 0});
    auto [m2, s2] = makeMRV({1, 1, 1, 1, 1, 1, 1, 1});
    auto r        = rmf::findChanged(m1, s1, m2, s2, 8);
    ASSERT_EQ(r.size(), 1);
    EXPECT_EQ(r[0].rAddr, 0);
    EXPECT_EQ(r[0].rSize, 8);
}
...
TEST(findChanged, PartialMiddle)
{
    ...
    auto [m1, s1] = makeMRV(std::move(a));
    auto [m2, s2] = makeMRV(std::move(b));
    auto r        = rmf::findChanged(m1, s1, m2, s2, 1);
    ASSERT_EQ(r.size(), 1);
...
TEST(findChanged, AdjacentMerges)
{
    ...
    auto [m1, s1] = makeMRV(std::move(a));
    auto [m2, s2] = makeMRV(std::move(b));
    auto r        = rmf::findChanged(m1, s1, m2, s2, 2);
    ASSERT_EQ(r.size(), 1);
    EXPECT_EQ(r[0].rAddr, 0);
    EXPECT_EQ(r[0].rSize, 4);
}
...
TEST(findChanged, CompareSizePartialLast)
{
    ...
    auto [m1, s1] = makeMRV(std::move(a));
    auto [m2, s2] = makeMRV(std::move(b));
    auto r        = rmf::findChanged(m1, s1, m2, s2, 4);
    ASSERT_EQ(r.size(), 2);
    EXPECT_EQ(r[0].rAddr, 0);
    EXPECT_EQ(r[0].rSize, 4);
    EXPECT_EQ(r[1].rAddr, 8);
...
TEST(findNumChanged, NoChange)
{
    ...
    auto [m1, s1] = makeMRV(std::move(data1));
    auto [m2, s2] = makeMRV(std::move(data2));
    auto r        = rmf::findNumChanged(m1, s1, m2, s2, uint32_t(1));
    EXPECT_TRUE(r.empty());
}
...
TEST(findNumChanged, BelowThreshold)
{
    ...
    auto [m1, s1] = makeMRV(std::move(data1));
    auto [m2, s2] = makeMRV(std::move(data2));
    auto r        = rmf::findNumChanged(m1, s1, m2, s2, uint32_t(5));
    EXPECT_TRUE(r.empty());
}
...
TEST(findNumChanged, Prealignment)
{
    ...
    auto [m1, s1] = makeMRV(std::move(data1), 0x1001, 1);
    auto [m2, s2] = makeMRV(std::move(data2), 0x1001, 1);
    auto r        = rmf::findNumChanged(m1, s1, m2, s2, uint32_t(5));
    ASSERT_EQ(r.size(), 1);
...
TEST(findNumChanged, SignedNegative)
{
    ...
    auto [m1, s1] = makeMRV(std::move(data1));
    auto [m2, s2] = makeMRV(std::move(data2));
    auto r        = rmf::findNumChanged(m1, s1, m2, s2, int32_t(1));
    EXPECT_TRUE(r.empty());
}
...
TEST(findNumUnchanged, NoChange)
{
    ...
    auto [m1, s1] = makeMRV(std::move(data1));
    auto [m2, s2] = makeMRV(std::move(data2));
    auto r        = rmf::findNumUnchanged(m1, s1, m2, s2, uint32_t(0));
    ASSERT_EQ(r.size(), 1);
    EXPECT_EQ(r[0].rAddr, 0);
    EXPECT_EQ(r[0].rSize, sizeof(uint32_t));
}
...
TEST(findNumUnchanged, AllChangedTooMuch)
{
    ...
    auto [m1, s1] = makeMRV(std::move(data1));
    auto [m2, s2] = makeMRV(std::move(data2));
    auto r        = rmf::findNumUnchanged(m1, s1, m2, s2, uint32_t(5));
    EXPECT_TRUE(r.empty());
}
...
{
    auto [m, s] = makeMRV({});
    auto r      = rmf::findString(m, s, "hello");
    EXPECT_TRUE(r.empty());
}

TEST(findString, NoMatch)
{
    auto [m, s] = makeMRV({1, 2, 3, 4});
    auto r      = rmf::findString(m, s, "hello");
    EXPECT_TRUE(r.empty());
}
...
TEST(findString, SingleMatch)
{
    ...
    auto [m, s]               = makeMRV(std::move(data));
    auto r                    = rmf::findString(m, s, "abc");
    ASSERT_EQ(r.size(), 1);
    EXPECT_EQ(r[0].rAddr, 0);
...
TEST(findNumExact, NotFound)
{
    ...
    auto [m, s] = makeMRV(std::move(data));
    auto r      = rmf::findNumExact(m, s, uint32_t(99));
    EXPECT_TRUE(r.empty());
}
...
TEST(findNumExact, SingleMatch)
{
    ...
    auto [m, s] = makeMRV(std::move(data));
    auto r      = rmf::findNumExact(m, s, uint32_t(3));
    ASSERT_EQ(r.size(), 1);
...
TEST(findNumExact, MultipleMatches)
{
    ...
    auto [m, s] = makeMRV(std::move(data));
    auto r      = rmf::findNumExact(m, s, uint32_t(7));
    ASSERT_EQ(r.size(), 3);
    EXPECT_EQ(r[0].rAddr, 0);
    EXPECT_EQ(r[1].rAddr, 2 * sizeof(uint32_t));
...
TEST(findNumExact, Prealignment)
{
    ...
    auto [m, s] = makeMRV(std::move(data), 0x1001, 1);
    auto r      = rmf::findNumExact(m, s, uint32_t(7));
    EXPECT_TRUE(r.empty());
}
...
TEST(findNumExact, SignedNegative)
{
    ...
    auto [m, s] = makeMRV(std::move(data));
    auto r      = rmf::findNumExact(m, s, int32_t(-5));
    ASSERT_EQ(r.size(), 1);
...
TEST(findNumInRange, NoneInRange)
{
    auto data   = makeNumData<uint32_t>({1, 2, 3});
    auto [m, s] = makeMRV(std::move(data));
    auto r      = rmf::findNumInRange(m, s, uint32_t(10), uint32_t(20));
    EXPECT_TRUE(r.empty());
}
...
TEST(findNumInRange, SingleInRange)
{
    ...
    auto [m, s] = makeMRV(std::move(data));
    auto r      = rmf::findNumInRange(m, s, uint32_t(4), uint32_t(6));
    ASSERT_EQ(r.size(), 1);
    EXPECT_EQ(r[0].rAddr, 1 * sizeof(uint32_t));
}
...
TEST(findNumInRange, AllInRange)
{
    ...
    auto [m, s] = makeMRV(std::move(data));
    auto r      = rmf::findNumInRange(m, s, uint32_t(1), uint32_t(5));
    ASSERT_EQ(r.size(), 3);
}
...
TEST(findNumInRange, BoundaryValues)
{
    ...
    auto [m, s] = makeMRV(std::move(data));
    auto r      = rmf::findNumInRange(m, s, uint32_t(5), uint32_t(10));
    ASSERT_EQ(r.size(), 3);
    EXPECT_EQ(r[0].rAddr, 0);
...
TEST(findNumInRange, SignedRange)
{
    ...
    auto [m, s] = makeMRV(std::move(data));
    auto r      = rmf::findNumInRange(m, s, int32_t(-5), int32_t(5));
    ASSERT_EQ(r.size(), 3);
    EXPECT_EQ(r[0].rAddr, 1 * sizeof(int32_t));
...
TEST(findUnchanged, Empty)
{
    auto [m1, s1] = makeMRV({});
    auto [m2, s2] = makeMRV({});
    auto r        = rmf::findUnchanged(m1, s1, m2, s2, 1);
    EXPECT_TRUE(r.empty());
}
...
{
    auto [m1, s1] = makeMRV({0, 0, 0, 0});
    auto [m2, s2] = makeMRV({1, 1, 1, 1});
    auto r        = rmf::findUnchanged(m1, s1, m2, s2, 1);
    EXPECT_TRUE(r.empty());
}
...
TEST(findUnchanged, AdjacentMerge)
{
    ...
    auto [m1, s1] = makeMRV(std::move(a));
    auto [m2, s2] = makeMRV(std::move(b));
    auto r        = rmf::findUnchanged(m1, s1, m2, s2, 2);
    ASSERT_EQ(r.size(), 1);
    EXPECT_EQ(r[0].rAddr, 0);
    EXPECT_EQ(r[0].rSize, 4);
}
