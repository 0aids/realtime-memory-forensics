...
namespace rmf::Utils::Detail
{
    ...
    bool SPMCQueue<T, MaxThreads>::tryEnqueue(T&& value)
    {
        ...
        if (produceIndex - consumeIndex >= size - 1)
        {
            ...
            rmf_Warning("Current size: {}", produceIndex - consumeIndex - 1);
            return false;
        }
    ...
    void SPMCQueue<T, MaxThreads>::enqueue(T&& value)
    {
        ...
        while (true)
        {
            produceIndex = m_produceIndex.load(std::memory_order_relaxed);
            consumeIndex = m_consumeCommitIndex.load(std::memory_order_acquire);
    ...
    Opt<T> SPMCQueue<T, MaxThreads>::tryDequeue()
    {
        ...
        uint64_t consumeIndex =
            m_consumeClaimIndex.fetch_add(1, std::memory_order_acquire);
...
namespace rmf::Utils
{
    ...
    std::future<ReturnType> ThreadPool::pushTask(Func func, Args&&... args)
    {
