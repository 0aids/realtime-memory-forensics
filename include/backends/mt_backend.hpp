...
namespace rmf::backends::mt
{
    ...
    struct Task
    {
        ...
        std::packaged_task<ReturnTypeOfT<CoreFuncType>()>
            packagedTask;
        using ReturnType = ReturnTypeOfT<CoreFuncType>;
    };
    ...
    template <typename CoreFuncType, typename... UniqueInputs>
                    ...
                    UniqueInputs... uInputs)
    {
        ...
        auto t   = Task<CoreFuncType>{.result       = {},
                                      .packagedTask = std::move(ptask)};
        t.result = t.packagedTask.get_future();
        return t;
    }
    template <typename CoreFuncType>
    auto initTasksList(CoreFuncType) {
        return std::vector<Task<CoreFuncType>>{};
    }


    template <typename CoreFuncType, typename... UniqueInputs>
    auto createMultipleTasks(CoreFuncType                  coreFunc,
                             const std::vector<CoreInputs> cInputsVec,
                             UniqueInputs... uInputs)
    {
        ...
        {
            tasksVec.push_back(
                createTask(coreFunc, cInputs, uInputs...));
        }
    ...
    class QueuedThreadPool
    {
      ...
      public:
        static constexpr auto TRYDURATION =
            std::chrono::milliseconds(10);
        const size_t pu_numThreads = 1;
        QueuedThreadPool(const size_t& numThreads) :
            pu_numThreads(numThreads)
        {
            ...
            for (size_t i = 0; i < pu_numThreads; i++)
            {
                ...
                m_threadsList.emplace_back(
                    std::thread(threadFunctions, std::ref(m_alive),
                                std::ref(m_tasksQueue)),
                    std::to_string(m_threadsList.size() + 1));

                // For debugging purposes.
                pthread_setname_np(
                    m_threadsList.back().first.native_handle(),
      ...
      public:
        ...
        {
            m_tasksQueue.enqueue([&packagedTask = task.packagedTask]()
                                 { packagedTask(); });
        }
      ...
      private:
        std::vector<std::pair<std::thread, std::string>>
                                m_threadsList;
        std::atomic<bool>       m_alive;
        utils::SPMCQueue<QTask> m_tasksQueue;

        static void threadFunctions(std::atomic<bool>&       alive,
                                    utils::SPMCQueue<QTask>& q)
        {
    ...
    template <typename CoreFuncType>
    ReturnTypeOfT<CoreFuncType> consolidateNestedTaskResults(
        std::vector<Task<CoreFuncType>>& tasks)
    {
    ...
    template <typename T>
    std::vector<T>
    consolidateNestedVector(std::vector<std::vector<T>>& vector)
    {
        size_t numElements = 0;
    ...
    divideSingleRegion(const MemoryRegionProperties& mrp,
                       size_t                        quantity);
    ...
    struct MultipleCoreInputs
    {
        ...
        const std::optional<std::vector<MemorySnapshotSpan>>
            snap1Vec = {};
        const std::optional<std::vector<MemorySnapshotSpan>>
            snap2Vec = {};
    };

    std::vector<MemorySnapshotSpan>
    divideSingleSnapshot(const MemorySnapshot& snap, size_t quantity);

    std::vector<CoreInputs>
    consolidateIntoCoreInput(const MultipleCoreInputs& c);

    // Converts a list of snapshots into its equivalent list of spans.
    std::vector<MemorySnapshotSpan>
    makeSnapshotSpans(const std::vector<MemorySnapshot>& snapVec);

    std::vector<MemorySnapshot> convertTasksIntoSnapshots(
        std::vector<Task<MemorySnapshot (*)(const CoreInputs&)>>&
            tasks);


// What a fucking disaster.
template <typename TaskFuncType>
static auto consolidateTasks(
    std::vector<Task<TaskFuncType>> &tasksList
) {
    using TaskType = std::decay_t<decltype(tasksList)>::value_type;
    using ReturnType = TaskType::ReturnType;

    std::vector<ReturnType> toReturn;
    for (auto &task : tasksList) {
        toReturn.push_back(task.result.get());
    }
    return toReturn;
}

template <typename TaskFuncType>
static auto consolidateTasksNestedResults(
    std::vector<Task<TaskFuncType>> &tasksList
) {
    using TaskType = std::decay_t<decltype(tasksList)>::value_type;
    // It normally returns a vector of mrps, so we want to get the mrps;
    using ReturnType = TaskType::ReturnType::value_type;

    std::vector<ReturnType> toReturn;
    for (auto &task : tasksList) {
        for (auto &value : task.result.get()) {
